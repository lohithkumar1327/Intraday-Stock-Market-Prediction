...
# --- Fetch Stock Data (no caching) ---
def load_data(symbol, start, end):
    try:
        df = yf.download(symbol, start=start, end=end)
        if df.empty:
            st.error(f"⚠️ No data found for '{symbol}'. It may be an invalid ticker or a network issue.")
        return df
...
# --- Auto Refresh Logic ---
st_autorefresh(interval=60_000, key="live_refresh")  # 60 seconds

# --- Final Chart (Limited Recent Data) ---
data = data.tail(180)
plt.clf()
